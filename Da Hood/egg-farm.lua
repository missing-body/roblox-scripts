-- this prob wont work no more seeming they removed eggs, but next easter it may work again.

rconsolename("eggfarm.lua ; seth.#0001 - ".. string.format("%02i:%02i %s", ((os.date("*t").hour % 24 - 1) % 12) + 1, os.date("*t").min, os.date("*t").hour % 24 < 12 and "AM" or "PM"));
rconsoleprint("@@CYAN@@");
...
local function FetchJobIds()
    ...
    repeat
        ...
        for i, v in next, ServersAPI["data"] do
            if v["id"] ~= game.JobId and v["playing"] ~= v["maxPlayers"] and tonumber(v["playing"]) ~= 0 then
